...
class SpectrumImage :
    def __init__( self, si, energy, adf=None ):
...
class LineProfile :
    def __init__( self, lp, energy, adf=None ):
    ...
    def update_spectrum1(self):
        ...
        if self.roi2_enabled:
            ...
            self.ui['roi2'].extents = (roi2[0], roi2[1], emin,emax)
        ...
        self.ui['slid_e_view'].set_val( ( self.energy[emin],self.energy[emax]) )
    ...
    def update_spectrum2(self):
        ...
        self.ui['roi1'].extents = (roi1[0], roi1[1], emin,emax)
    ...
    def onclick_fbsub(self):
        if (self.int_check and self.fit_check):
            self.lp_bsub = bg.bgsub_SI_fast( self.lp, self.energy, self.edge, self.r1, fit_options=self.fit_options)
    ...
    def onclick_bsub(self):
        if (self.int_check and self.fit_check):
            if self.fit_options.lc:
                _,self.lp_bsub = bg.bgsub_SI( self.lp, self.energy, self.edge, fit_options=self.fit_options)
            else:
                self.lp_bsub =   bg.bgsub_SI( self.lp, self.energy, self.edge, fit_options=self.fit_options)
            self.update_image()
